import { cn } from '@/lib/cn'

interface StarRatingProps {
  rating: number | null
  reviews?: number | null
  size?: 'sm' | 'md' | 'lg'
  showCount?: boolean
  className?: string
}

export default function StarRating({
  rating,
  reviews,
  size = 'md',
  showCount = true,
  className,
}: StarRatingProps) {
  if (!rating) return null

  const fullStars = Math.floor(rating)
  const partial = rating - fullStars
  const emptyStars = 5 - fullStars - (partial > 0 ? 1 : 0)

  const starSizes = { sm: 'w-3.5 h-3.5', md: 'w-4 h-4', lg: 'w-5 h-5' }
  const textSizes = { sm: 'text-xs', md: 'text-sm', lg: 'text-base' }
  const starSize = starSizes[size]

  return (
    <div className={cn('flex items-center gap-1.5', className)}>
      <div className="flex items-center">
        {Array.from({ length: fullStars }).map((_, i) => (
          <StarIcon key={`full-${i}`} className={cn(starSize, 'text-gold')} fill />
        ))}
        {partial > 0 && (
          <StarIcon className={cn(starSize, 'text-gold')} partial={partial} />
        )}
        {Array.from({ length: emptyStars }).map((_, i) => (
          <StarIcon key={`empty-${i}`} className={cn(starSize, 'text-neutral-300')} />
        ))}
      </div>
      <span className={cn('font-semibold text-neutral-800', textSizes[size])}>
        {rating.toFixed(1)}
      </span>
      {showCount && reviews && (
        <span className={cn('text-neutral-500', textSizes[size])}>
          ({reviews.toLocaleString()})
        </span>
      )}
    </div>
  )
}

function StarIcon({
  className,
  fill,
  partial,
}: {
  className?: string
  fill?: boolean
  partial?: number
}) {
  if (partial !== undefined) {
    return (
      <svg className={className} viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient id={`partial-${partial}`}>
            <stop offset={`${partial * 100}%`} stopColor="currentColor" />
            <stop offset={`${partial * 100}%`} stopColor="#D4D1CC" />
          </linearGradient>
        </defs>
        <path
          fill={`url(#partial-${partial})`}
          d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"
        />
      </svg>
    )
  }

  return (
    <svg className={className} viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
      <path
        fill={fill ? 'currentColor' : 'none'}
        stroke={fill ? 'none' : 'currentColor'}
        strokeWidth={fill ? 0 : 1.5}
        d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"
      />
    </svg>
  )
}
